...
app=Flask(__name__)
version=1
data=pd.read_csv(f'data{version}.csv')
...
def universities():
    ...
    return render_template('universities.html',data=data,datalen=len(data),ld=math.ceil(len(data)/20),title='Universities',collegenames=collegenames)
...
def findmycollege():
    ...
    if request.method == "POST":
        ...
        datalen=len(df)
        return render_template('findmycollege.html',title='Filter',data=df,datalen=datalen,userdata=usercollege,userdatalen=len(usercollege))
